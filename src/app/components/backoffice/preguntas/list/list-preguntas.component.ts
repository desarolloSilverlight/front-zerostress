...
export class ListPreguntasComponent implements OnInit {
  displayedColumns: string[] = ['id', 'descripcion', 'orden', ' '];
  ...
  constructor(private preguntasService: PreguntasService, private actRoute: ActivatedRoute) {
    this.parent = this.actRoute.snapshot.params.parent;
  }

  ngOnInit(): void {
    const pregunta = new Preguntas();
    pregunta.idFormulario = this.parent;
    this.preguntasService.list(pregunta).toPromise().then(resp => {
  ...
  singularFilter(event: Event) {
    ...
    const t: Preguntas = new Preguntas();
