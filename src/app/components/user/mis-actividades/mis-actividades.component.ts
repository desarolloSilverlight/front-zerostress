...
export class MisActividadesComponent implements OnInit, AfterViewInit {
  ...
  constructor(private router: Router, private dialog: MatDialog, private consultaService: ConsultasService, private globals: GlobalsUser,
  ...
  ngOnInit(): void {
    ...
    this.consultaService.listVi(consulta).pipe(map(m => {
      ...
      m = m.filter(t => t.status === 'P');
      return m;
    })).toPromise().then(resp => {
      Swal.close();
      this.consultas = resp;
      // if (this.consultas.length == 0 || this.consultas == undefined) {
      //   Swal.fire({
      //     allowOutsideClick: false,
      //     icon: 'info',
      //     text: 'Ni tienes actividades por hacer'
      //   });
      ...
      setTimeout(() => {
        this.consultas.forEach(con => {
          ...
          con.contenidos.forEach(s => {
            console.log(s.titulo, '-->', s.titulo.length);
          });
